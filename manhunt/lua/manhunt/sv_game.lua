...
function Manhunt.StartGame()
    ...
    Manhunt.Config.GameTime = math.Clamp(Manhunt.Config.GameTime or 30, 1, 120)
    Manhunt.Config.Interval = math.Clamp(Manhunt.Config.Interval or 3, 0.5, 10)
    Manhunt.Config.Rounds = math.Clamp(Manhunt.Config.Rounds or 1, 1, 10)
    if Manhunt.Config.Interval > Manhunt.Config.GameTime then
        Manhunt.Config.Interval = Manhunt.Config.GameTime
    end
...
function Manhunt.CheckInterval()
    ...
    -- Chase mode doesn't use classic interval scans
    if Manhunt.IsChaseMode() then
        timer.Remove("Manhunt_IntervalCheck")
        return
    end
...
function Manhunt.TriggerEndgameMechanic()
    ...
    -- Chase mode doesn't use classic endgame mechanics
    if Manhunt.IsChaseMode() then return end
...
function Manhunt.StopGame()
    ...
    -- Also stop chase if active
    if Manhunt.Chase and Manhunt.Chase.Active then
        Manhunt.Chase.StopGame()
    end
...
hook.Add("PlayerDeath", "Manhunt_PlayerDeath", function(victim, inflictor, attacker)
    ...
    -- Chase mode handles its own death/loss logic
    if Manhunt.IsChaseMode() then return end
...
net.Receive("Manhunt_RequestStart", function(len, ply)
    ...
    -- Route to chase mode if selected
    if Manhunt.IsChaseMode() then
...
-- Rate limit lobby sync requests per player
Manhunt._lobbySyncCooldown = Manhunt._lobbySyncCooldown or {}

net.Receive("Manhunt_LobbySync", function(len, ply)
    local sid = ply:SteamID()
    if Manhunt._lobbySyncCooldown[sid] and Manhunt._lobbySyncCooldown[sid] > CurTime() then return end
    Manhunt._lobbySyncCooldown[sid] = CurTime() + 1
...
concommand.Add("manhunt_endgame", function(ply, cmd, args)
    -- Only allow admins
    if IsValid(ply) and not Manhunt.IsAdmin(ply) then
