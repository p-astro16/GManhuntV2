...
if SERVER then
    ...
    util.AddNetworkString("Manhunt_TutorialAllDone")
    ...
    -- ============================================================
    -- CONFIG PERSISTENCE: Save/Load to file
    -- ============================================================
    local CONFIG_PATH = "manhunt/config.json"

    function Manhunt.SaveConfig()
        if not file.IsDir("manhunt", "DATA") then
            file.CreateDir("manhunt")
        end
        local data = {
            GameTime = Manhunt.Config.GameTime,
            Interval = Manhunt.Config.Interval,
            Rounds = Manhunt.Config.Rounds,
            TutorialEnabled = Manhunt.Config.TutorialEnabled,
            ZoneEnabled = Manhunt.Config.ZoneEnabled,
            Gamemode = Manhunt.Gamemode or 0,
        }
        file.Write(CONFIG_PATH, util.TableToJSON(data, true))
    end

    function Manhunt.LoadConfig()
        ...
        Manhunt.Config.GameTime = math.Clamp(tonumber(data.GameTime) or 30, 1, 120)
        Manhunt.Config.Interval = math.Clamp(tonumber(data.Interval) or 3, 0.5, 10)
        Manhunt.Config.Rounds = math.Clamp(tonumber(data.Rounds) or 1, 1, 10)
        Manhunt.Config.TutorialEnabled = data.TutorialEnabled ~= false
        Manhunt.Config.ZoneEnabled = data.ZoneEnabled ~= false
        Manhunt.Gamemode = math.Clamp(tonumber(data.Gamemode) or 0, 0, 1)
        print("[Manhunt] Config loaded from file")
    end
