--[[
    ...
    Press SPACE to advance, ESC to skip
...
function Manhunt.Tutorial.ShowTeamPicker()
    ...
    btnSkip.DoClick = function()
        ...
        state.showPicker = false
        Manhunt.Tutorial.PendingThisRound = false
        Manhunt.Tutorial.CompletedThisRound = true  -- prevent re-trigger
        -- Start the cinematic intro now that tutorial is skipped
...
function Manhunt.Tutorial.Stop()
    ...
    -- Now start the cinematic intro (white fade -> FPV)
    if Manhunt.StartCinematicIntro then
        Manhunt.StartCinematicIntro()
    end
end
...
-- Key handling: SPACE to advance, ESC to skip
...
hook.Add("HUDPaint", "Manhunt_TutorialHUD", function()
    ...
    draw.SimpleText("[SPACE] Next  |  [ESC] Skip Tutorial", "Manhunt_HUD_Small", sw / 2, sh - barH - 10, Color(150, 150, 150, hintAlpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
